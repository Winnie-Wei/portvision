/**
 * @module port/layer/mapPop 地图相关的弹框层
 */
define(function (require) {
    ...
    var RealTimeWarnPop = require('mod/realTimeWarnPop/realTimeWarnPop');
    var DeviceProfilePop = require('mod/deviceProfilePop/deviceProfilePop');
    var DeviceDetailPop = require('mod/deviceDetailPop/deviceDetailPop');
    var EquipmentPop = require('mod/equipmentPop/index');
    var BIGraphyPop = require('mod/biGraphy/biGraphy');
    var BIGraphyCenterPop = require('widget/biGraphyCenterPop/biGraphyCenterPop');
    ...
    var BoatBottomPop = require('mod/popup/boatBottomPop');
    var BoatCenterPop = require('mod/popup/boatCenterPop');
    var YardCenterPop = require('mod/popup/yardCenterPop');
    ...
    POPLAYER.prototype = $.extend(new McBase(), {
        ...
        initPops: function () {
            ...
            // 最新告警消息弹框 RealTimeWarnPop
            if (!this._newWarns) {
                this._newWarns = RealTimeWarnPop({
                    'zr': this._zr,//zr
                    'observer': this._observer,//观察者
            ...
            // };
            // 设备概况消息内弹框 DeviceProfilePop
            if (!this._infoPop) {
                this._infoPop = DeviceProfilePop({
                    'observer': this._observer,//观察者
                    'showScale': this.showScale,//缩放倍数
                    'initScale': this.initScale
                });
            };
            // 设备详情消息内弹框 DeviceDetailPop
            if (!this._infoDetailPop) {
                this._infoDetailPop = DeviceDetailPop({
                    'zr': this._zr,//zr
                    'observer': this._observer,//观察者
                    'showScale': this.showScale,//缩放倍数
                    'initScale': this.initScale
            ...
            // BI图表中心弹框
            if (!this._biGraphyCenterPop) {
                this._biGraphyCenterPop = BIGraphyCenterPop({
                    'dom': $('#J_biGraphyModal'),
                    'zr': this._zr,//zr
                    'observer': this._observer,//观察者
                    'showScale': this.showScale,//缩放倍数
                ...
                if (this._biGraphyCenterPop) {
                    this._biGraphyCenterPop.render();
                }
            };
            // 全局搜索
        ...
        hidePop: function () {
            ...
            this._boxAnimatePop && this._boxAnimatePop.hide();
            this._boxDetailPop && this._boxDetailPop.hide();
            // this._boxMarker && this._boxMarker.hide();
        },
        /*
         * 隐藏bi分析弹框，响应地图放大缩小
         */
        ...
        showPop: function (type, args) {
            ...
            switch (type) {
                ...
                case 'boatDetail':
                    this.hidePop()
                    var boatBottomPop = BoatBottomPop({
                        'data': args,
                        'elementId': '#J_blockPop',
                        'bayElementId': '#J_centerPop',
                        'observer': this._observer
                    });
                    boatBottomPop.render();
                    break;
                case 'bayDetail':
                    //console.log( args );
                    this.hidePop();
                    var yardCenterPop = YardCenterPop({
                        //'data':data,
                        'elementId': '#J_centerPop',
                        'type': 'center',
                        'observer': this._observer
                    });
                    yardCenterPop.getBayDetailByParam(args.yard, args.bay);
                ...
                case 'boatBayDetail':
                    this.hidePop();
                    var boatCenterPop = BoatCenterPop({
                        //'data':data,
                        'elementId': '#J_centerPop',
                        'type': 'center',
                        'observer': this._observer
                    });
                    boatCenterPop.getBoatBayDetailByParam(args.vesselCode, args.bay);
                    break;
                case 'boxDetail':
                    this.hidePop();
                    ...
                    centerPopup.getContainerDetail(args.boxId);
                    break;
                case 'equipmentDetail':
                    ...
                    var centerDetailPopup = CenterDetailPopup({
                        'data': data,
                        'elementId': '#J_centerPop',
                        'type': 'center',
                        'observer': this._observer
        ...
        setInfoPopPositionByDevice: function (device) {
            console.log('mapPop setInfoPopPostionByDevice', device);
