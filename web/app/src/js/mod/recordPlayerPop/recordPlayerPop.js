...
define(function (require) {
    ...
    var playerTemp = '<div class="j-wrapper">' +
        '<div class="j-oc">' +
        '</div>' +
        // '<div class="j-showTime"></div>' +
        '<div>' +
        '<div class="j-player">' +
        '<div class ="j-date">' +
        '<div>' +
        ...
        '<button type="button" class="btn btn-default btn-sm active">1倍</button>' +
        '<button type="button" class="btn btn-default btn-sm">4倍</button>' +
        '<button type="button" class="btn btn-default btn-sm">8倍</button>' +
        '<button type="button" class="btn btn-default btn-sm">16倍</button>' +
        '</div>' +
        '</div>' +
        '<div class="r">' +
        '<button type="button" class="btn btn-default btn-sm active zt">暂停</button>' +
        '<button type="button" class="btn btn-default btn-sm qd">启动</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>';
    var upDateTemp = '<div class="btn-group lr" role="group" aria-label="...">' +
    ...
    var downDateTemp = '<li class="previous active"><</li>' +
        '{{for dateArr}}' +
    ...
    var workingClassTemp = '<div class="preWorkingClass workingClass">' +
        '<p>上一工班</p>' +
        '<p class="active"><<</p>' +
    ...
    var noWorkingClassTipTemp = '<div class="j-workingTip hide">' +
        '<div>' +
        ...
        '</div>' +
        '</div>';
    var initTipTemp = '<div class="j-workingTip">' +
        '<div>' +
        '<h3 class="pull-left">提示</h3>' +
        '<span class="j-close pull-right">X</span>' +
        '<p>请选择目标日期或搜索目标对象进行回放</p>' +
        '</div>' +
        '</div>';
    ...
    RECORDPLAYERPOP.prototype = $.extend(new McBase(), {
        ...
        bindEvent: function () {
            ...
            $('#player')
                ...
                .on('click', '.j-date li.previous', function () {
                    that.setDatelr($(this), 'l');
                })
                .on('click', '.j-date li.next', function () {
                    that.setDatelr($(this), 'r');
                })
                .on('click', '.j-date li:not(.previous,.next,.active)', function () {
                    // that.dateData = $.extend(true, {}, that.dateDataCopy);
                    var upCurtext = $('.j-date .btn.active').text();
                    switch (upCurtext.charAt(upCurtext.length - 1)) {
                        case '年':
                            that.dateData.showYear = parseInt($(this).text());
                            var curMondays = that.judgeDate(that.dateData.showYear, 2);
                            that.dateData.showDay = ((that.dateData.showMonth === 2) && (that.dateData.showDay > curMondays)) ? curMondays : that.dateData.showDay;
                            var newYmd = that.dateData.showYear + '-' + that.dateData.showMonth + '-' + that.dateData.showDay;
                            if (that.compareDate(newYmd, that.ymdData.ymd)) {
                                that.dateData.showYear = that.ymdData.showYear;
                                that.dateData.showMonth = that.ymdData.showMonth;
                                that.dateData.showDay = that.ymdData.showDay;
                            }
                            that.renderUpDate();
                            $('.j-date .btn:first-child').addClass('active');
                            break;
                        case '月':
                            that.dateData.showMonth = parseInt($(this).text());
                            var curMondays = that.judgeDate(that.dateData.showYear, that.dateData.showMonth);
                            (that.dateData.showDay > curMondays) && (that.dateData.showDay = curMondays);
                            var newYmd = that.dateData.showYear + '-' + that.dateData.showMonth + '-' + that.dateData.showDay;
                            if (that.compareDate(newYmd, that.ymdData.ymd)) {
                                that.dateData.showYear = that.ymdData.showYear;
                                that.dateData.showMonth = that.ymdData.showMonth;
                            ...
                            break;
                        case '日':
                            that.dateData.showDay = parseInt($(this).text());
                            that.renderUpDate();
                    ...
                    $('.j-refresh').removeClass('hide')
                    that.initWorkingClassTime();
                    that.renderWorkingClass(that.dateData);
                    // that.useSetDate(that.dateData.stime);
                    that.setFuture();
                    that.judgeWorkingClass();
                })
                .on('click', '.j-refresh', function () {
                    that.dateData.flagC = false;
                    that.observer._recordPop._filterPop.setTimeChange(that.getShowDate());
                    that.setTimeChange(that.getShowDate());
                    that.useSetDate(that.dateData.stime);
                    $(this).addClass('hide');
                    $('.j-date ul').html('');
                ...
                .on('mouseover', '.bar', function (ev) {
                    var mousePos = that.mousePosition(ev);
                    var hms = that.getTime(mousePos, $(this));
                ...
                .on('click', '.bar', function (ev) {
                    ...
                    var hms = that.getTime($('.bar>span').offset().left + 6, $('.bar'));
                    that.useSetDate(hms);
                })
                ...
                .on('click', '.j-btn>.l .btn', function () { // 播放速率
                    $(this).addClass('active')
                        .siblings().removeClass('active');
                    that.observer.setSpeed(parseInt($(this).text()));
                })
                ...
                .on('click', '.j-btn>.r .zt', function () { // 暂停回放
                    that.observer.stopDeviceAnimator();
                ...
                .on('click', '.j-btn>.r .qd', function () { // 启动回放
                    ...
                    var hms = that.getTime($('.bar>span').offset().left + 6, $('.bar'));
                    that.useSetDate(hms);
                    that.clickedQd = false;
                    // that.observer.startDeviceAnimator();
                    that.observer._isPause = false;
                    that.observer.startPlayback();
                })

                // 切换工班
                .on('click', '.j-progressBar p.active', function () {
                    if (!$('.j-refresh').hasClass('hide')) return; // 在选择日期时候不能切换工班
                    that.transWorkingClass($(this));
        ...
        getShowDate: function () {
            return this.dateData.showYear + '-' + this.dateData.showMonth + '-' + this.dateData.showDay;
        },
        // 根据进来的时间设置进度条上未来
        setFuture: function () {
        ...
        setTimeChange: function (newDate) {
            ...
            $('.j-showTime').html(this.observer.dateToString(newDate));
        ...
        judgeWorkingClass: function () {
            var that = this;
            if (!((that.dateData.showYear + '-' + that.dateData.showMonth) === that.workingClassInfo.ym)) {
                return;
            }
            $('#tipPop').html(noWorkingClassTipTemp);
            $('.j-workingTip').addClass('hide');
            var flag = false;
            $.each(that.workingClassInfo.data, function (idx, item) {
                if (that.dateData.showDay === item.DD) {
                    if (that.dateData.stime === '0:00:00' && item.HH === 'M') {
                        flag = true;
                    }
                    if (that.dateData.stime === '8:00:00' && item.HH === 'A') {
                        flag = true;
                    }
                    if (that.dateData.stime === '16:00:00' && item.HH === 'N') {
                        flag = true;
                    }
                }
        ...
        transWorkingClassPart: function (flag) {
            this.transHour(flag);
            this.renderWorkingClass(this.dateData);
            this.setTimeChange(this.getShowDate() + ' ' + this.dateData.stime);
            this.useSetDate(this.dateData.stime);
        },
        transWorkingClass: function (obj) {
            var that = this;
            var title = obj.siblings(':first-child').text();
            var hour = parseInt(that.dateData.stime.split(':')[0]);
            var curMondays = that.judgeDate(this.dateData.showYear, this.dateData.showMonth);
            // var $list = $('.j-date ul li');
            // var $child = $('.j-date ul li:nth-child(2)');
            var newYmd = that.getShowDate();
            var initYmd = that.ymdData.ymd;

            if (title === '下一工班' && that.compareDate(newYmd, initYmd)) {
                if (hour > 8) return;
                if (hour <= 8 && (new Date()).getHours() + 1 <= parseInt(that.dateData.etime.split(':')[0])) {
                    return;
                }
            }
            if (title === '下一工班' && hour <= 8) {
                that.transWorkingClassPart('n');
                that.setFuture();
                // return;
            }
            if (title === '下一工班' && hour > 8) {
                that.dateData.showDay += 1;
                if (that.dateData.showDay > curMondays) {
                    that.dateData.showDay = 1;
                    that.dateData.showMonth += 1;
                    if (that.dateData.showMonth > 12) {
                        that.dateData.showMonth = 1;
                        that.dateData.showYear += 1;
                    }
                }
                that.renderUpDate();
                // that.judgeActive($list, $child);
                that.setFuture();
                that.transWorkingClassPart('above');
            }

            if (title === '上一工班' && hour >= 8) {
                that.transWorkingClassPart('p');
                // return;
            }
            if (title === '上一工班' && hour < 8) {
                that.dateData.showDay -= 1;
                if (that.dateData.showDay < 1) {
                    that.dateData.showDay = that.judgeDate(that.dateData.showYear, that.dateData.showMonth - 1);
                    that.dateData.showMonth -= 1;
                    if (that.dateData.showMonth < 1) {
                        that.dateData.showMonth = 12;
                        that.dateData.showYear -= 1;
                    }
                }
                that.renderUpDate();
                // that.judgeActive($list, $child);
                that.transWorkingClassPart('below');
            }
            that.judgeWorkingClass();
        ...
        transHour: function (flag) {
            var that = this;
            that.dateData.stime = that.getHour(that.dateData.stime, flag);
            that.dateData.etime = that.getHour(that.dateData.etime, flag);
            $.each(that.dateData.mtime, function (idx, obj) {
                that.dateData.mtime[idx] = that.getHour(obj, flag);
            })
        },
        ...
        getTime: function (mousePos, obj) {
            ...
            var time = stime + (etime - stime) * proportion;
            this.clickedQd && (time += this.observer._requestInterval * this.observer._playSpeed);
            var time = this.transTime(time, 2);
            return time;
        },
